        ...
        <v-row>
            <v-col>
                <v-card class="pa-5">
                    ...
                    <v-data-table-server
                        v-model:page="page"
                        v-model:items-per-page="productCategoriesPerPage"
                        class="pt-5"
                        :headers="headers"
                        :items="productCategories"
                        :items-length="productCategoriesTotalCount"
                            ...
                            <v-menu rounded>
                                ...
                                >
                                    <v-list density="compact" class="text-secondary">
                                        <Link :href="route('admin.product-category.edit', { productCategory: item.id })">
                                            <v-list-item
...
const productCategoriesTotalCount = ref(0);
const headers = [
    {
        sortable: false,
        key: 'name',
...
function loadItems(): void {
    loading.value = true;
    axios.get(route('admin.product-category.fetch', {
        page: page.value,
        per_page: productCategoriesPerPage.value,
        search: search.value,
    }))
    .then((response) => {
        productCategories.value = response.data.data;
        productCategoriesTotalCount.value = response.data.meta.total;
    })
    .catch((error) => {
        console.error(error);
        addNotification('A apărut o eroare neașteptată în timpul încărcării datelor', 'error');
        productCategories.value = [];
        productCategoriesTotalCount.value = 1;
    })
    .finally(() => {
        loading.value = false;
    });
